 ...
 * ================================================================
 * No existe ni un solo logotipo de cliente en el proyecto:
 *
 *   /public/clientes  ·  /public/clients  ·  /public/logos   no existen
 *   clients.ts        ·  campo `logo?` declarado, 0 cuentas lo usan
 *   Sanity            ·  la consulta GROQ ni siquiera pide un logo
 *
 * La version anterior rellenaba ese hueco con las DOS PRIMERAS LETRAS del
 * nombre dentro de un cuadrado. Eso no identificaba nada —"EL" salia igual
 * para El Horno y para El Cobayo, "CO" se repetia tres veces— y leia como
 * una maqueta sin terminar, que es lo contrario de lo que tiene que hacer
 * una franja de prueba social.
 *
 * Asi que se quitan los monogramas y se muestra lo unico que SI es real y
 * verificable: el nombre de cada empresa. Es honesto y funciona: responde
 * la unica pregunta de este bloque —"¿alguien ya confio en ustedes?"— sin
 * fingir tener material que no tenemos.
 *
 * ================================================================
 * COMO ACTIVAR EL MODO LOGOS (cuando lleguen los archivos)
 * ================================================================
 *   1. Deja cada logo en /public/clientes/<slug>.svg
 *      Monocromo, preferiblemente SVG. Si es raster, WebP o AVIF.
 *   2. Añade `logo: "/clientes/<slug>.svg"` a esa cuenta en clients.ts
 *
 * El componente ya los pinta: si una cuenta trae `logo`, se renderiza la
 * imagen; si no, su nombre. No hace falta rediseñar nada.
 *
 * OJO: mientras haya mezcla (unas con logo y otras sin el) la franja se ve
 * inconsistente. Cuando llegue el material real hay que decidir el umbral
 * a partir del cual se pasa a modo logos y se dejan fuera las cuentas sin
 * archivo. Esa decision es del sprint de Casos, no de este.
 *
 * ================================================================
 * VELOCIDAD
 * ================================================================
 * La duracion NO es un numero fijo: se calcula a partir de cuantas cuentas
 * hay, para que la velocidad real en px/s se mantenga aunque se agreguen o
 * quiten. Misma velocidad optica en movil que en escritorio: acelerar la
 * marquesina en pantallas pequeñas es un tic, no una decision.
 */
      ...
      <div className="relative mt-6 md:mt-7">
        ...
        >
          {tira.map((c, i) => {
            ...
            // esto anunciaria las 26 empresas dos veces seguidas.
              ...
              >
                {c.logo ? (
                  ...
                  <img
                    ...
                    loading="lazy"
                    decoding="async"
                    className="h-6 w-auto max-w-[120px] object-contain opacity-70 md:h-7 md:max-w-[150px]"
                  />
                ) : (
